...
def get_patch_embeddings(image):
    ...
    vision_inputs = {
        k: v.cuda() for k, v in inputs.items() if k.startswith("pixel_values")
    }
    with torch.no_grad():
...
def get_text_embedding(text):
    ...
    text_inputs = {
        k: v.cuda() for k, v in inputs.items() if k in ["input_ids", "attention_mask"]
    }
    with torch.no_grad():
...
def overlay_heatmap_on_image(
    image, similarity_scores, grid_size, alpha=0.5, cmap="jet"
):
    """
...
def run_pipeline(
    ...
    heatmap_img = overlay_heatmap_on_image(
        image.copy(), similarities, (cols, rows), alpha=0.5
    )
