...
def get_patch_embeddings(image):
    ...
    vision_inputs = {
        k: v.to(DEVICE) for k, v in inputs.items() if k.startswith("pixel_values")
    }
    ...
    resized_image_tensor = processor.image_processor(image, return_tensors="pt")[
        "pixel_values"
    ][0]
...
def select_top_k_patches(patch_tokens, text_embedding, top_k_percentage):
    ...
    sims = torch.nn.functional.cosine_similarity(
        patch_tokens, text_embedding[0], dim=-1
    )
...
# ---- Main Pipeline ----
def run_pipeline(
    image_url, text_prompt, top_k_percentage=10, visualize=True, save_path=None
):
    # Load image from URL
    image = Image.open(requests.get(image_url, stream=True).raw).convert("RGB")

    # Run patch extraction
    patch_tokens, resized_image, grid_size, original_size, resized_size = (
        get_patch_embeddings(image)
    )
    text_embedding = get_text_embedding(text_prompt)
    top_indices, similarities = select_top_k_patches(
        patch_tokens, text_embedding, top_k_percentage
    )
    ...
    vis_original = visualize_on_original(
        image.copy(), top_indices, grid_size, resized_size
    )
