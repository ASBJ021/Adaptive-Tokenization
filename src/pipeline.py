...
from transformers import AutoModel, AutoProcessor, AutoModelForZeroShotImageClassification
...
def compare_big_clip_full_vs_selected(original_image, top_indices, grid_size, resized_size, prompt):
    ...
    input_canvas = big_processor(images=canvas, return_tensors="pt")["pixel_values"].to(DEVICE)
    full_input = big_processor(images=original_image, return_tensors="pt")["pixel_values"].to(DEVICE)
    text_inputs = big_processor(text=prompt, return_tensors="pt").to(DEVICE)

    with torch.no_grad():
        t0 = time.time()
        canvas_feat = big_model.get_image_features(input_canvas)
