...
def get_model_info(model_name):
    try:
        ...
        model_size_mb = sum(
            t.element_size() * t.nelement() for t in model.state_dict().values()
        ) / (1024**2)
...
def compare_results(results, ground_truth):
    ...
    for alias, res in results:
        ...
        print(
            f"  Model Size     : {res.get('model_size_mb', 'N/A'):.2f} MB"
            if res.get("model_size_mb")
            else ""
        )
        print("  Label Scores   :")
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare zero-shot image classification models."
    )
    parser.add_argument(
